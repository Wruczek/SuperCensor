...
public class SCConfigManager2 {
    ...
    public static void load() {
        ...
        if (SCMain.getInstance().getResource("messages/messages_" + messagesLanguage + ".yml") == null) {
            SCLogger.logError(
                    "Cannot find \"messages_" + messagesLanguage + "\" file! Changing to default \"messages_en.yml\"!",
        ...
        if (!disabledRulesFolder.exists()) {
            ...
            try {
                IOUtils.copyResource("configs/Readme_disabled_rulesfolder.txt",
                        new File(disabledRulesFolder, "Readme.txt"), false);
            } catch (IOException e) {
        ...
        try {
            messages_original = YamlConfiguration
                    .loadConfiguration(SCMain.getInstance().getResource("messages/messages_en.yml"));
            config_original = YamlConfiguration
                    .loadConfiguration(SCMain.getInstance().getResource("configs/config.yml"));
        } catch (Exception e) {
